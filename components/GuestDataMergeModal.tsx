...
export const GuestDataMergeModal: React.FC<GuestDataMergeModalProps> = ({ guestPlans, guestHistoryCount, onMerge, onDiscard }) => {
  ...
  const mergeButtonText = useMemo(() => {
    ...
    if (count === 0 && guestHistoryCount === 0) return 'בחר מה להוסיף';
    if (count === 0) return 'הוסף היסטוריה לחשבון';
    if (count === 1) return 'הוסף אימון נבחר והיסטוריה';
    return `הוסף ${count} אימונים והיסטוריה`;
  }, [selectedPlanIds.length, guestHistoryCount]);
  
  const hasGuestData = guestPlans.length > 0 || guestHistoryCount > 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-gray-300 mt-3 text-center">מצאנו נתונים במכשיר זה. בחר מה תרצה להוסיף לחשבונך.</p>
        
        {hasGuestData && (
          <div className="text-center text-gray-400 mt-3 text-sm">
            {guestPlans.length > 0 && <p>• {guestPlans.length} תוכניות אימונים</p>}
            {guestHistoryCount > 0 && <p>• {guestHistoryCount} אימונים שהושלמו</p>}
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-6">
          <button
            onClick={handleMerge}
            disabled={selectedPlanIds.length === 0 && guestHistoryCount === 0}
